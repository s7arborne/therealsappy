    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
      {projects.map(p => (
        <a key={p.id} href={p.url || "#"} target={p.url ? "_blank" : undefined} rel="noopener noreferrer"
          ...
          <span style={{ display: "flex", gap: 12, alignItems: "center", color: "var(--muted)", fontSize: 12.5, whiteSpace: "nowrap" }}>
            {p.tag && <span style={{ border: "1px solid var(--line)", borderRadius: 999, padding: "3px 10px", fontSize: 10.5, textTransform: "uppercase", letterSpacing: ".05em" }}>{p.tag}</span>}
            <span>↗</span>
          </span>
        </a>
      ))}
