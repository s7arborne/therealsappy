...
export function ThoughtNote({ thought }: { thought: Thought }) {
  return (
    <article className="glass" style={{ padding: 26 }}>
      <div style={{ display: "flex", gap: 12, color: "var(--muted)", fontSize: 12, textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 14 }}>
      ...
      <h3 style={{ fontSize: 22, fontWeight: 700, letterSpacing: "-.02em", marginBottom: 12 }}>{thought.title}</h3>
      <p style={{ color: "var(--muted)", marginBottom: 14 }}>
        {thought.bodyMd.replace(/\*\*/g, "").split("\n\n")[0].slice(0, 160)}…
      </p>
      <Link href={`/thoughts/${thought.slug}`} className="thought-more"
        style={{ fontSize: 13, color: "var(--fg)", borderBottom: "1px solid var(--accent)", paddingBottom: 2 }}>
        Read the full note →
      </Link>
    </article>
  );
